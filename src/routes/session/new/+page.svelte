<script lang="ts">
	...
	import { SessionWizard } from '$components/session';
	import type { MetricConfig, TrackingSession } from '$types';
	...
	// Simple form state (used when not using wizard)
	let name = $state('');
	...
	async function handleWizardComplete(sessionData: Partial<TrackingSession>) {
		isSubmitting = true;
		error = '';
		...
		try {
			const session = await sessions.create({
				...
				enabledMetrics: sessionData.enabledMetrics!,
				reminderConfig: sessionData.reminderConfig!
			});
...
<div class="space-y-6">
	...
	{:else}
		<!-- Simple Form -->
		<form onsubmit={(e) => { e.preventDefault(); handleSimpleSubmit(); }} class="space-y-6">
			<!-- Session Name -->
			<Input
				label="Session Name"
